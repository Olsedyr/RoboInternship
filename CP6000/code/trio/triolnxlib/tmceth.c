...
long OpenSocket(int iIndex)
{
   ...
   /* Open a socket for primary communciations */
	...
	{
#ifdef TMC_DEBUG
		TMCTrace("Could not open a socket. Return code <%ld>.\n", errno);
#endif
		return TMCERROR_DRIVER;
	}
   ...
   /* Bind to the socket - needed for 2 way communication (peer-to-peer) and to use setsockopt */
	...
	{
#ifdef TMC_DEBUG
		TMCTrace("Could not bind to socket. Return code <%ld>.\n", errno);
#endif
	...
	if (controller[iIndex].controllerinfo.hardwareinfo.socketinfo.fProtocol == EthernetProtocolTCP)
	{
		...
		{
#ifdef TMC_DEBUG
			TMCTrace("Could not set socket option for TCP_NODELAY. Return code <%ld>.\n", errno);
#endif
		}
		...
		{
#ifdef TMC_DEBUG
			TMCTrace("Could not get socket option for TCP_NODELAY. Return code <%ld>.\n", errno);
#endif
		}
		...
		{
#ifdef TMC_DEBUG
			TMCTrace("TCP_NODELAY option was not set.\n", errno);
#endif
		}
	}

	SockAddressS.sin_family = AF_INET;
	SockAddressS.sin_port = htons(TRIO_TCPIP_PORT);
   ...
   /* Connect to the host address */
	...
	{
#ifdef TMC_DEBUG
		TMCTrace("Could not connect to host address. Return code <%ld>.\n", errno);
#endif
		close(controller[iIndex].socket);
		return TMCERROR_DRIVER;
	}
   ...
   /* Open a socket for multi-cast communciations */
	...
	{
#ifdef TMC_DEBUG
		TMCTrace("Could not open a multicast socket. Return code <%ld>.\n", errno);
#endif
   ...
   // Bind to the socket - needed for 2 way communication (peer-to-peer) and to use setsockopt
	...
	{
#ifdef TMC_DEBUG
		TMCTrace("Could not bind to multicast socket. Return code <%ld>.\n", errno);
#endif
	...
	if (rc != SOCKET_ERROR)
	{
		if (TTL != 1)
		{
			...
			{
#ifdef TMC_DEBUG
				TMCTrace("Could not set socket option for IP_MULTICAST_TTL Return code <%ld>.\n", errno);
#endif
			}
		}
	}
   ...
   // Join the multi-cast group
	...
	{
#ifdef TMC_DEBUG
		TMCTrace("Could not join to multicast group. Return code <%ld>.\n", errno);
#endif
		printf("\nCould not join to multicast group. Return code <%ld>.\n", errno);
		close(controller[iIndex].socketMulticast);
		return TMCERROR_DRIVER;
	}
...
long ETHERNETClear(int iIndex)
{
	...
	rc = send(controller[iIndex].socket, "\r", 1, MSG_OOB);

   /* Probem? */
	if (rc == SOCKET_ERROR)           
	{
#ifdef TMC_DEBUG
		TMCTrace("   Write error. Error code <%d>.\n", errno);
#endif
		if (errno ==ECONNRESET)
			rc = TMCERROR_DEVICE_DISCONNECTED;
		else
			rc = TMCERROR_DRIVER;
	}
	else
...
long ReadSocket(int iIndex, PCHAR pchResponse, ULONG cbResponse, PULONG pulBytesRead)
{
   ...
   /* Is available data? */
	...
	{
#ifdef TMC_DEBUG
		TMCTrace("   Read error (select). Error code <%d>.\n", errno);
#endif
		if (errno == ECONNRESET)
			rc = TMCERROR_DEVICE_DISCONNECTED;
		else
   ...
   /* Probem? */
	...
	{
#ifdef TMC_DEBUG
		TMCTrace("   Read error. Error code <%d>.\n", errno);
#endif
		if (errno == ECONNRESET)
			rc = TMCERROR_DEVICE_DISCONNECTED;
		else
...
LONG FAR TRIOCALL TMCAssignIPAddress(PCONTROLLERINFO pcontrollerinfo, int fVerbose)
{
	long        rc = 0L;
	short       ServerPort = 67;
	short       ClientPort = 68;
	...
	{
#ifdef TMC_DEBUG
		TMCTrace("IP Address is not valid.\n");
#endif
		if (fVerbose)
   ...
   /* Bind to the socket - needed for 2 way communication (peer-to-peer) */
	...
	{
#ifdef TMC_DEBUG
		TMCTrace("Could not bind to a Windows socket. Return code <%ld>.\n", errno);
#endif
		if (fVerbose)
	...
	{
#ifdef TMC_DEBUG
		TMCTrace("Could not set socket options. Return code <%ld>.\n", errno);
#endif
		if (fVerbose)
	...
	ulTimeoutTime = TMCGetTime() + 15000L;
	while (1)
	{
		...
		if (!WillReadBlock(sock))
		{
			...
			{
#ifdef TMC_DEBUG
				TMCTrace("Error returned from \"recvfrom\". Return code <%ld>.\n", errno);
				TMCTrace("An error occurred attempting to read a BOOTP packet. Return code %ld.\n", errno);
#endif
				if (fVerbose)           
				{
			...
			if (0 < (nLen = FillBootpRep((unsigned char *)szBuf, rc /* contains bytes read */, ulIPAddress, &usModel, &usSerialNumber)))
			{
				...
				if (usModel)
				{
					...
					if (usModel == MODEL_224)
						strcpy(szModel, MC224);
					else
						strcpy(szModel, "unknown");
#ifdef TMC_DEBUG
					TMCTrace("An Ethernet controller (model %s) with serial number %u was found.\n",
				...
				{
#ifdef TMC_DEBUG
					TMCTrace("An Ethernet controller with serial number %u was found.\n",
						usSerialNumber);
#endif
					if (fVerbose)
            ...
            /* Assign IP address then cancel search */
				if (usSerialNumber == pcontrollerinfo->ulSerialNumber)
				{
					...
					{
#ifdef TMC_DEBUG
						TMCTrace("Error returned from \"sendto\". Return code <%ld>.\n", errno);
						TMCTrace("An error occurred attempting to write a BOOTP packet. Return code %ld.\n", rc);
#endif
						if (fVerbose)
						{
					...
					else
					{
						bConfigured = TRUE;
#ifdef TMC_DEBUG
						TMCTrace("Ethernet controller serial number %u was configured with IP Address %s.\n",
