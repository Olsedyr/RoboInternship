...
long SERIALReadData(int iIndex, PCHAR pchResponse, ULONG cbResponse, PULONG pulBytesRead)
{
   ...
   *pulBytesRead = 0L;
   ...
   else if (rc == -1)
   {
      ...
      {
#ifdef TMC_DEBUG
         TMCTrace("   Read error. Error number <%d>.\n", errno);
#endif
         rc = TMCERROR_DRIVER;
       }
   }
...
long SERIALWriteData(int iIndex, PSZ pchCommand, ULONG cbCommand, PULONG pulBytesWritten)
{
   ...
   *pulBytesWritten = 0L;
...
#ifdef TMC_DEBUG
   TMCTrace("   Waiting for OK to send.\n");
#endif   
   ...
   do
   {
      ...
      {
#ifdef TMC_DEBUG      
         TMCTrace("   Time-out waiting for OK to send.\n");
#endif         
         rc = TMCERROR_TIMEOUT;
         break;
      }
...
long OpenPort(int iIndex)
{
   ...
   {
#ifdef TMC_DEBUG
      TMCTrace("   Invalid comm port. Setting comm port to 1.\n");
#endif
   ...
   {
#ifdef TMC_DEBUG
      TMCTrace("   Could not open comm port. Error number <%d>.\n", errno);
#endif
   ...
   {
#ifdef TMC_DEBUG
      TMCTrace("   Could not get comm port attributes. Error number <%d>.\n", errno);
#endif
   ...
   CommAttribs.c_cflag = CREAD | CS7 | CRTSCTS | CSTOPB | PARENB;
...
#ifdef TMC_DEBUG
         TMCTrace("   Invalid baud rate. Setting baud rate to 9600.\n");
#endif
   ...
   {
#ifdef TMC_DEBUG
      TMCTrace("   Could not set input comm speed. Error number <%d>.\n", errno);
#endif
   ...
   {
#ifdef TMC_DEBUG
      TMCTrace("   Could not set ouput comm speed. Error number <%d>.\n", errno);
#endif
   ...
   {
#ifdef TMC_DEBUG
      TMCTrace("   Could not set comm port attributes. Error number <%d>.\n", errno);
#endif
...
long WriteAChar(int iIndex, CHAR ch)
{
   ...
   {
#ifdef TMC_DEBUG
      TMCTrace("   Write error. Error number <%d>.\n", errno);
#endif
   }
