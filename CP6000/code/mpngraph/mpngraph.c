...
{
  FILE *fp;
  char func[256], s[256],arg1[128], arg2[128], include[256], fn[384];
  char buffer[256];
  int version=0,idx=0, idx2=0, i, imax, clone;
  ln=0;
  strcpy(fn, filename);
  ...
  if(strlen(include)>0)
  {
    sprintf(fn,"%s%s",databasePath,include);
    fclose(fp);
    fp = fopen(fn,"r");
  }
  ...
  while(fgets(buffer, 256, fp)!=NULL)
  {
    ...
    else if(strncmp(func, "SETSTATE", 8)==0)
    {
      ...
      {    
        ...
        if(idx2>=2) //check if the transition was not allready added
        {
          for(i=1;i<idx2;i++)
          {
                if(strncmp(statemachine->state[idx].transition[i].name, arg1, sizeof(arg1))==0)
                  clone++;
          }
        }
        if(clone==0)
    ...
    else if(strncmp(func, "TIMEOUT", 7)==0)
    {
      ...
      if(idx2>=2) //check if the transition was not allready added
      {
        for(i=1;i<idx2;i++)
        {
           if(strncmp(statemachine->state[idx].transition[i].name, arg2, sizeof(arg2))==0) 
              clone++;
        }
      }
      if(clone==0)
