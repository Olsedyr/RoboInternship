...
int generateSplinePath(int maxidx
                      ...
                      ,tmpnPosition *position);
# 7 "spline.c" 2
void tknot(int n, int k, float *x);
void tbasis(int c,float t,int npts,float *x,float n[]);
# 19 "spline.c"
void tknot(int numberofkeypoints, int splineorder, float *knotTable)
{
  int i;
...
void tableInit(int numberofkeypoints, int splineorder, int numberofsamples
              ,float *knotTable, float *basisTable, int force)
{
   ...
   ||numberofsamples!=oldnumberofsamples)
  {
    ...
    t = 0.0;
   stepsize = ((float)knotTable[numberofkeypoints+splineorder-1])/((float)(numberofsamples-1));
...
# 152 "spline.c"
int tbspline(int numberofkeypoints, int splineorder, int numberofsamples, float *b, float *p)
{
static float basisTable[40000];
static float knotTable[500 +20];
  int i,icount,tidx,basisIdx;
  float max;
  int nplusc;
  float stepsize,t;
  if(numberofsamples*splineorder>40000) return 0;
  nplusc = numberofkeypoints + splineorder;
  tableInit(numberofkeypoints, splineorder, numberofsamples, knotTable, basisTable, 1);
  t=0.0;
 stepsize = ((float)knotTable[nplusc-1])/((float)(numberofsamples-1));
  for (icount = 0; icount< numberofsamples; icount++)
  {
    p[icount] = 0.0;
    max=0.0;
