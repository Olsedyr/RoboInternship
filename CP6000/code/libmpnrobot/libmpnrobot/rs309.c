...
void expandPath309BoptIn(tmpnRobot *robot, tmpnPathDef *ptrpath, int forceItem, const char *ppscriptname, int loadgroup) 
{
  ...
  tmpnFloat maxheight = 2120.0;
  tmpnFloat minheight = 340.0;
  ...
  if (ptrpath->pathtype==20) {
    ...
    viavectpos4.y = 450*bar*bar+200*hori*bar;
    viavectpos4.z = 10+20*bar*bar; 
    
    viavecthast4.y= 190*bar*bar+100*hori*bar;
    viavecthast4.z= 10+50*bar*bar+20*hori*bar; 
  
    // viapoints with respect to fromFrame:
    //viavectpos.x=-22;
    viavectpos.y=100+foo*foo*90+foo*foo*foo*foo*230;
    viavectpos.z=-310+foo*foo*60;
    
    //viavecthast.x=12;
    viavecthast.y=-foo*foo*50-foo*foo*foo*foo*90;
  ...
  //********************************************************************
    ...
    //viavectpos1.x=-20;
    viavectpos1.y=152+foo*foo*25+foo*foo*foo*foo*150;
    viavectpos1.z=-80-bar*bar*bar*230-110*hori*bar-50*bar;
    
    //viavecthast1.x=-12;
    viavecthast1.y=30+foo*foo*17+foo*foo*foo*foo*40;
    viavecthast1.z=-80-bar*bar*bar*100-30*hori*bar-25*hori;
    
    // viapoint with respect to toFrame:
    //viavectpos2.x=-130;
    viavectpos2.y=240+bar*bar*bar*420+260*hori*bar+60*bar-180*foo*foo*foo;
    viavectpos2.z=18+12*hori+foo*foo*foo*90;
    
    //viavecthast2.x=75;
    viavecthast2.y=-100-bar*bar*bar*130-50*hori*bar-10*bar+70*foo*foo*foo;
    viavecthast2.z=-15-15*foo*foo-65*foo*foo*foo-10*hori;
  ...
  //********************************************************************
    ...
    tmpnVector viavectpos3={0.000000, 63.789558, -28.260008, 0.000000, 0.000000, 0.00000};
    tmpnVector viavecthast3={0.000000, 46.884247, -46.712524, 0.000000, 0.000000, 0.00000};
    //maxtime: 991.000000
    ...
    for (i=0;i<500;i++) {
      ...
      // viavecthast3.x=viahastopt.x+uniform(-hastdlim/3.0,hastdlim/3.0);
...
//***********************************************************************
//*                                                                     *
//* Automatically generates optimal path for Out trajectories of 309    *
//* No check for collisions!!!                                          *
//*                                                                     *
//***********************************************************************
...
void expandPath309BoptOut(tmpnRobot *robot, tmpnPathDef *ptrpath, int forceItem, const char *ppscriptname, int loadgroup) 
{
  ...
  if (ptrpath->pathtype==23) {  
    ...
    //tmpnVector viavectpos={0.000000, 55.783112, 4.179654, 0.000000, 0.000000, 0.000000};
    //tmpnVector viavecthast={0.000000, -36.474766, -0.832042, 0.000000, 0.000000, 0.000000};
    //maxtime: 1066.000000

    tmpnVector viavectpos={6.443994, 59.992313, 9.619900, 0.000000, 0.000000, 0.000000};
    tmpnVector viavecthast={-0.447798, -44.077312, 8.925069, 0.000000, 0.000000, 0.000000};
    //maxtime: 1349.000000
    ...
    for (i=0;i<500;i++) {
     ...
     viavecthast.x=viahastopt.x+uniform(-hastdlim/3.0,hastdlim/3.0);
    ...
    printf("tmpnVector viavectpos={%f, %f, %f, %f, %f, 0.000000};\n",viavectopt.x,viavectopt.y,viavectopt.z,viavectopt.v,viavectopt.w);
    printf("tmpnVector viavecthast={%f, %f, %f, %f, %f, 0.000000};\n",viahastopt.x,viahastopt.y,viahastopt.z,viahastopt.v,viahastopt.w);
    printf("//maxtime: %f\n",maxtime);
  ...
  //********************************************************************
    ...
    // viapoints with respect to fromFrame:
    //viavectpos1.x=-156;
    viavectpos1.y=200+200*bar+500*bar*bar*bar;
    viavectpos1.z=-30-40*bar-25*foo*foo*foo;

    //viavecthast1.x=-70;   
    viavecthast1.y=95+65*bar+100*bar*bar*bar;
    viavecthast1.z=-10-40*bar+20*bar*bar*bar*bar-55*foo*foo*foo;
 
    // viapoint with respect to toFrame:
    //viavectpos2.x=-18;
    viavectpos2.y=150+bar*bar*bar*100+190*foo*foo*foo*foo;
    viavectpos2.z=50+bar*bar*80;
    
    //viavecthast2.x=10.2;
    viavecthast2.y=-80-bar*bar*bar*40-80*foo*foo*foo*foo;
    viavecthast2.z=-8-65*bar*bar;
  ...
  //********************************************************************
    ...
    //viavectpos3.x=-48;
    viavectpos3.y=150+foo*foo*foo*foo*170+bar*bar*bar*100;
    viavectpos3.z=110+bar*bar*bar*150;
    
    //viavecthast3.x=-23;
    viavecthast3.y=50+foo*foo*foo*foo*100+bar*bar*bar*100;
    viavecthast3.z=130+bar*bar*bar*100;
