...
void expandPath308BoptIn(tmpnRobot *robot, tmpnPathDef *ptrpath, int forceItem, const char *ppscriptname, int loadgroup) 
{
  ...
  //********************************************************************
    ...
    //viavectpos1.x=-20;
    viavectpos1.y=152+foo*50+foo*foo*40+foo*foo*foo*foo*90;
    viavectpos1.z=-80-bar*bar*bar*230-110*hori*bar-50*bar;
    
    //viavecthast1.x=-12;
    viavecthast1.y=30+foo*30+foo*foo*25+foo*foo*foo*foo*10;
    viavecthast1.z=-80-bar*bar*bar*130-25*hori*bar;
    
    // viapoint with respect to toFrame:
    //viavectpos2.x=-130;
    viavectpos2.y=240+bar*bar*bar*420+260*hori*bar+60*bar+30*bar*bar-110*foo*foo*foo*foo;
    viavectpos2.z=20+foo*foo*foo*90;
    
    //viavecthast2.x=75;
    viavecthast2.y=-90-bar*bar*bar*120-40*hori*bar-30*bar-20*bar*bar+50*foo*foo*foo*foo;
    viavecthast2.z=-20-70*foo*foo*foo;
  ...
  //********************************************************************
    ...
    tmpnVector viavectpos3={0.000000, 59.251717, -18.838118, 0.000000, 0.000000, 0.00000}; //cartesian mm and degrees
    tmpnVector viavecthast3={0.000000, 29.298870, -41.058132, 0.000000, 0.000000, 0.00000}; //cartesian mm and degrees per sec.
    //maxtime: 1132.000000
...
//***********************************************************************
//*                                                                     *
//* Automatically generates optimal path for Out trajectories of 308    *
//* No check for collisions!!!                                          *
//*                                                                     *
//***********************************************************************
...
void expandPath308BoptOut(tmpnRobot *robot, tmpnPathDef *ptrpath, int forceItem, const char *ppscriptname, int loadgroup) 
{
  ...
  if (ptrpath->pathtype==23) {  
    ...
    tmpnVector viavectpos={0.000000, 52.051022, 3.848759, 0.000000, 0.000000, 0.000000}; //cartesian mm and degrees
    tmpnVector viavecthast={0.000000, -34.205738, -9.498440, 0.000000, 0.000000, 0.000000}; //cartesian mm and degrees per sec.
    //maxtime: 1207.000000
  ...
  //********************************************************************
    ...
    // viapoints with respect to fromFrame:
    viavectpos1.x=-75;
    viavectpos1.y=200+670*bar*bar*bar+220*bar*bar;
    viavectpos1.z=-30;

    viavecthast1.x=-45;   
    viavecthast1.y=95+90*bar*bar*bar+15*foo*foo*foo*foo+100*bar*bar;
