...
COMMANDSTRUCT *expandSubPathKatrineIn(tmpnRobot *robot, COMMANDSTRUCT *ptrcmd, int *level, int forceItem, const char *ppscriptname, int loadgroup)
{
  ...
  exectime = 1200;
  dist=fabs(homeFrame->y-fromFrame->y-moveItem->wcp.y);
  exectime+=dist*0.80;
  dist=fabs(homeFrame->z-fromFrame->z);
  exectime+=dist*0.80;
  ...
  ptrcmd=addCommandString(ptrcmd,s,level);
    ...
    corner[0]=ptrcmd=convertsplit(ptrcmd,0,0,0,90,90,0,"home"
                        ,0,((0)>=(moveItem->wcp.y+fromFrame->y+bcl-homeFrame->y)?(0):(moveItem->wcp.y+fromFrame->y+bcl-homeFrame->y)),0,90,90,0,"home"
                        ,robot,-1,level,"home","case",1);

    corner[1]=ptrcmd=convertsplit(ptrcmd
                        ,0,((0)>=(moveItem->wcp.y+fromFrame->y+bcl-homeFrame->y)?(0):(moveItem->wcp.y+fromFrame->y+bcl-homeFrame->y)),0,90,90,0,"home"
                        ,moveItem->wcp.x,((homeFrame->y-fromFrame->y)>=(moveItem->wcp.y+bcl)?(homeFrame->y-fromFrame->y):(moveItem->wcp.y+bcl)),moveItem->wcp.z
                        ,moveItem->wcp.v,moveItem->wcp.w,0,moveItem->from
                        ,robot,-1,level,moveItem->from,"case",1);

    corner[2]=ptrcmd=convertsplit(ptrcmd,moveItem->wcp.x,((homeFrame->y-fromFrame->y)>=(moveItem->wcp.y+bcl)?(homeFrame->y-fromFrame->y):(moveItem->wcp.y+bcl)),moveItem->wcp.z
                        ,moveItem->wcp.v,moveItem->wcp.w,0,moveItem->from
    ...
    corner[4]=ptrcmd=convertsplit(ptrcmd,moveItem->wcp.x,moveItem->wcp.y,moveItem->wcp.z,moveItem->wcp.v,moveItem->wcp.w,0,moveItem->from
                        ,moveItem->wcp.x,moveItem->wcp.y,moveItem->wcp.z,moveItem->wcp.v,moveItem->wcp.w,0,moveItem->from
                        ,robot,3,level,moveItem->from,"case",1);
  ...
  dist=fabs(homeFrame->y-fromFrame->y-moveItem->wcp.y);
  exectime+=dist*1.15;
  dist=fabs(toFrame->z-fromFrame->z);
  exectime+=dist*1.15;
  ...
  ptrcmd=addCommandString(ptrcmd,s,level);
    ...
    corner[5]=ptrcmd=convertsplit(ptrcmd,moveItem->wcp.x,moveItem->wcp.y,moveItem->wcp.z,moveItem->wcp.v,moveItem->wcp.w,0,moveItem->from
                        ,moveItem->wcp.x,moveItem->wcp.y,moveItem->wcp.z,moveItem->wcp.v,moveItem->wcp.w,0,moveItem->from
                        ,robot,4,level,moveItem->from,"case",1);
    ...
    corner[7]=ptrcmd=convertsplit(ptrcmd,moveItem->wcp.x,((homeFrame->y-fromFrame->y)>=(moveItem->wcp.y+bcl)?(homeFrame->y-fromFrame->y):(moveItem->wcp.y+bcl)),moveItem->wcp.z
                        ,moveItem->wcp.v,moveItem->wcp.w,0,moveItem->from
                        ,0,((homeFrame->y-toFrame->y)>=(moveItem->wcp.y+bcl+fromFrame->y-toFrame->y)?(homeFrame->y-toFrame->y):(moveItem->wcp.y+bcl+fromFrame->y-toFrame->y)),0,90,0,0,moveItem->to
                        ,robot,-1,level,moveItem->from,"case",1);
...
COMMANDSTRUCT *expandSubPathKatrineOut(tmpnRobot *robot, COMMANDSTRUCT *ptrcmd, int *level, int forceItem, const char *ppscriptname, int loadgroup)
{
  ...
  float dist,rzminus,rzplus,rxminus,rxplus,hightfactor,ry;
  float rzminus0,rzplus0,rxminus0,rxplus0;
  float rzminus1,rzplus1,rxminus1,rxplus1;
...
# 289 "rs306.c"
  exectime = 2500;
  dist=fabs(homeFrame->y-toFrame->y-moveItem->wcp.y);
  exectime+=dist*1.15;
  dist=fabs(fromFrame->z-toFrame->z);
  exectime+=dist*1.15;
   ...
   if((moveItem->wcp.y+bcl)+toFrame->y-fromFrame->y>100)
    {
     ...
     ,robot,4,level,moveItem->to,"case",1);
      ...
      corner[12]=ptrcmd=convertsplit(ptrcmd,moveItem->wcp.x,moveItem->wcp.y+ry,moveItem->wcp.z,moveItem->wcp.v,moveItem->wcp.w,0,moveItem->to
       ...
       ,robot,18,level,moveItem->to,"case",1);
      blendCorner3(robot,corner[5]);
    }
    else
    {
  ...
  dist=fabs(homeFrame->y-toFrame->y-moveItem->wcp.y);
  exectime+=dist*0.80;
  dist=fabs(homeFrame->z-toFrame->z);
  exectime+=dist*0.80;
  ...
  ptrcmd=addCommandString(ptrcmd,s,level);
    ...
    corner[19]=ptrcmd=convertsplit(ptrcmd,moveItem->wcp.x,moveItem->wcp.y,moveItem->wcp.z,moveItem->wcp.v,moveItem->wcp.w,0,moveItem->to
                                         ,moveItem->wcp.x,moveItem->wcp.y,moveItem->wcp.z,moveItem->wcp.v,moveItem->wcp.w,0,moveItem->to
     ,robot,4,level,moveItem->to,"case",0);
   ...
   ptrcmd=addCommandString(ptrcmd,"POSITIONHINT 1",level);
    ...
    corner[22]=ptrcmd=convertsplit(ptrcmd,moveItem->wcp.x,moveItem->wcp.y+50,moveItem->wcp.z,moveItem->wcp.v,moveItem->wcp.w,0,moveItem->to
      ,moveItem->wcp.x,((homeFrame->y-toFrame->y)>=(moveItem->wcp.y+bcl)?(homeFrame->y-toFrame->y):(moveItem->wcp.y+bcl)),moveItem->wcp.z
      ,moveItem->wcp.v,moveItem->wcp.w,0,moveItem->to
      ,robot,-1,level,moveItem->to,"case",1);

    corner[23]=ptrcmd=convertsplit(ptrcmd,moveItem->wcp.x,((homeFrame->y-toFrame->y)>=(moveItem->wcp.y+bcl)?(homeFrame->y-toFrame->y):(moveItem->wcp.y+bcl)),moveItem->wcp.z
      ,moveItem->wcp.v,moveItem->wcp.w,0,moveItem->to
      ,0,((0)>=(moveItem->wcp.y+bcl+toFrame->y-homeFrame->y)?(0):(moveItem->wcp.y+bcl+toFrame->y-homeFrame->y)),0,90,90,0,"home"
      ,robot,-1,level,moveItem->to,"case",1);
